...
> 최종 갱신: 2026-04-20 (2) (Game HUD + 세션 정산 + Range 업그레이드 v2 포팅 완료, §8 참조)
...
| 주제 | 문서 |
|---|---|
| 캐릭터 + 어빌리티 9종 | [Sys-Character.md](Sys-Character.md) |
| 무기 해금/강화 + 회전톱날 | [Sys-Weapon.md](Sys-Weapon.md) |
| 보석 드랍·채집, mineTarget 승리 | [Sys-Gem.md](Sys-Gem.md) |
| 구글시트 신규 시트 스키마 | [GoogleSheetsGuide_v2Addendum.md](archive/GoogleSheetsGuide_v2Addendum.md) |
...
| # | 작업 | 영향 | 문서 |
|---|---|---|---|
| 1 | `PlayerData.Currency` → `Ore` + `Gem` 분리 | DataManager, SessionResultUI | [Sys-Gem.md §5](Sys-Gem.md) |
| 2 | `UpgradeData` 이중 재화 필드 추가 + 신규 항목 (`mine_target`, `gem_drop`, `gem_speed`) | UpgradeManager 확장 | [GoogleSheetsGuide_v2Addendum.md §2](archive/GoogleSheetsGuide_v2Addendum.md) |
| 3 | `WeaponUnlock` 상태 `PlayerData`에 추가 | bool[] 또는 HashSet | [Sys-Weapon.md §3](Sys-Weapon.md) |
| 4 | `WeaponUpgradeData` SO + UpgradeManager의 무기별 누적 보너스 | WeaponBase가 조회 | [Sys-Weapon.md §4](Sys-Weapon.md) |
| 5 | `CharacterData` SO + `CharacterSelectUI` (MachineSelect 리네임/확장) | UI 거의 그대로 | [Sys-Character.md §2](Sys-Character.md) |
| 6 | `AbilityData` SO + `IAbilityRunner` 9종 구현 | **가장 무거움**. 시각효과+로직 | [Sys-Character.md §4](Sys-Character.md) |
| 7 | 인게임 보석 드랍/채집 | BugController.OnDeath + GemPickup | [Sys-Gem.md §2](Sys-Gem.md) |
| 8 | `mineTarget` 승리 조건 | MachineController·SessionManager | [Sys-Gem.md §4](Sys-Gem.md) |
| 9 | 회전톱날 무기 | WeaponBase 상속 신규 클래스 | [Sys-Weapon.md §6](Sys-Weapon.md) |
| 10 | `GoogleSheetsImporter` 확장 (4시트) | 기존 패턴 복사 | [GoogleSheetsGuide_v2Addendum.md §6](archive/GoogleSheetsGuide_v2Addendum.md) |
...
**무기별 강화**는 `WeaponUpgradeData` SO로 분리. UpgradeType 열거형에 섞지 않음. (→ [Sys-Weapon.md](Sys-Weapon.md))
...
| 7 | 보석 드랍/채집 (`GemDrop`/`GemPickup`) | ✅ | `Scripts/Pickup/Gem.cs` + `GemDropSpawner.cs`. v2 2종 보석 (일반 #aadfff +1 / 엘리트 #ffd700 +5). 세션 종료 시 일괄 정산. 상세 [Sys-Gem.md §10](Sys-Gem.md) |
| 8 | `mineTarget` 승리 조건 | ✅ | `MachineController.CheckSessionEnd`가 `IsMiningTargetReached`만 체크 (연료 조건 제거). 2026-04-20 (2) |
| 9 | 회전톱날(`SawWeapon`) | ✅ | `Scripts/Weapon/Saw/SawWeapon.cs` + 블레이드 프리펩 빌더 + WeaponPanel 슬롯. 상세 [Sys-Weapon.md §7](Sys-Weapon.md) |
| 10 | `GoogleSheetsImporter` 4시트 확장 | ❌ | 우선순위 낮음 |

### Game HUD + 세션 정산 완료 (2026-04-20 (2))

| 작업 | 상태 | 파일 |
|---|---|---|
| 연료 시스템 완전 제거 | ✅ | `MachineController`, `MachineData`, `GameEvents`, `MachineStatusUI`, `DebugManager` 등 10+ 파일 |
| v2 세션 광석(sessionOre = mined*0.5 + score*0.5) | ✅ | `MachineController._sessionOre` + `OnBugScoreEarned` 이벤트 |
| v2 정산 규칙 (승리=100%, 패배=50%, 나가기=0) | ✅ | `MachineController.CheckSessionEnd` |
| TopBarHud (v2 `#ig-topbar`) | ✅ | `Scripts/UI/HUD/TopBarHud.cs` + `Scripts/Editor/TopBarHudSetupEditor.cs` (메뉴 자동 생성) |
| Result Popup 단일 통합 (v2 `#resultPanel`, 500/200ms 지연, 성공/실패 아이콘) | ✅ | `Scripts/UI/SessionResultUI.cs` + `Scripts/Editor/ResultPanelSetupEditor.cs` |
| 보석 2종 (일반 +1 파랑 / 엘리트 +5 금색), 크기 0.7→1.2 | ✅ | `Scripts/Pickup/Gem.cs`, `GemDropSpawner.cs` |
| 저격총 Range → 에임 동적 확장 (`SetRangeMultiplier`) | ✅ | `Scripts/Aim/AimController.cs`, `SniperWeapon.cs` |
| 레이저 Range → 빔 반경 확장 (독립 값) | ✅ | `Scripts/Weapon/Laser/LaserWeapon.cs`, `LaserBeam.cs` |
| Title 진입 HubPanel 기본화 | ✅ | `TitleUI.Start`, Hub TopBar 옵션/종료 버튼 |

### 버그 수정 (2026-04-20 (2))

- **UpgradeType enum 재배열로 깨진 에셋 정수값 정정** — 연료 제거로 enum 중간 2값 삭제 → 뒤 값 밀림. `Upgrade_MiningRate/MiningTarget/GemDrop/GemSpeed` 에셋 정수값 수정. enum에 명시적 값 부여 + 재발 방지 주석
- **HubController 목표 라벨 미갱신** — `OnUpgradePurchased` 구독 추가
- **StoreSessionResult 이중 적립** — 내부 `AddOre/AddGems` 제거, 책임 분리

### Hub → Game 연결 완료 (2026-04-19)

| 작업 | 상태 | 파일 |
|---|---|---|
| `MachineController`에 `UpgradeManager` 보너스 합산 (HP/MineRate/MineTarget/Armor) | ✅ | `Machine/MachineController.cs` |
| 선택 캐릭터 → `CharacterData.DefaultMachine` 주입 | ✅ | `OutGame/CharacterRegistry.cs` (신규 싱글턴) |
| 미해금 무기 Game 씬 비활성 (`WeaponBase.TryDisableIfLocked`) | ✅ | `Weapon/WeaponBase.cs` + 5종 무기 Start 가드 |
| 무기별 강화 4종 (Sniper/Bomb/Gun/Laser)에 `WeaponUpgradeManager` 보너스 적용 | ✅ | Saw 패턴 복사, 투사체 `Initialize` effective 오버로드 추가 |
| AimRingBinder 4종 — 미해금 무기 감지 시 원호 영구 숨김 | ✅ | `Aim/*AimRingBinder.cs` |
...
| 2026-04-19 (2) | Hub → Game 연결: UpgradeManager 보너스, 무기 해금 필터, 무기 강화 4종 적용, 캐릭터 DefaultMachine 주입 |
| 2026-04-19 (3) | 보석 드랍/채집 구현: `OnBugDied` 이벤트 + `GemDropSpawner`/`Gem` + HUD 광석·보석 카운터 에디터 |
| 2026-04-20 | 문서 일괄 갱신 |
| 2026-04-20 (2) | 연료 제거, mineTarget 승리 단일화, TopBarHud/ResultPanel(v2 포팅), 세션 광석 정산(mined×0.5+score×0.5, 승리100%/패배50%/나가기0%), 보석 2종(일반+1/엘리트+5), 저격총·레이저 Range 업그레이드, Hub 첫 진입 화면, enum 재배열 버그 수정 |
