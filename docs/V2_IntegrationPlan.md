...
> 최종 갱신: 2026-04-20 (Hub→Game 연결 + 보석 드랍/채집 완료, §8 참조)
...
| 7 | 보석 드랍/채집 (`GemDrop`/`GemPickup`) | ✅ | `Scripts/Pickup/Gem.cs` + `GemDropSpawner.cs`. `OnBugDied` 이벤트 구독 → 확률 롤 → 월드 스프라이트 스폰 → 2초 호버 채집 → 즉시 `AddGems(1)`. 상세 [GemMiningSystem.md §10](GemMiningSystem.md) |
| 8 | `mineTarget` 승리 조건 | ❌ | 데이터만 있음 (`MachineController.MiningTarget` 노출 완료), 세션 종료 로직 미전환 |
