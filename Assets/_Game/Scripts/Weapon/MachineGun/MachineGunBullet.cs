...
namespace DrillCorp.Weapon.MachineGun
{
    ...
    /// 풀링됨 — Despawn 은 Destroy 대신 BulletPool.Return 호출.
    /// </summary>
    public class MachineGunBullet : MonoBehaviour
    {
        ...
        private GameObject _prefabKey;    // BulletPool 복귀용 원본 프리팹 참조
        private TrailRenderer _trail;     // 풀 재사용 시 이전 궤적 Clear 필요
        private bool _trailLookup;
        ...
        /// prefabKey 는 풀 복귀 시 키 — null 이면 레거시 경로(Destroy 폴백).
        /// </summary>
        public void Initialize(Vector3 direction, MachineGunData data, LayerMask bugLayer,
                               float effectiveDamage, GameObject prefabKey)
        {
            _data = data;
            _bugLayer = bugLayer;
            ...
            _consumed = false;           // 풀 재사용 시 반드시 리셋
            ...
            // TrailRenderer 가 있으면 이전 재사용 시 남아있던 궤적 제거
            // (SetActive 토글만으로는 points 가 유지돼 순간이동 선이 그려짐)
            if (!_trailLookup)
            {
        ...
        private void Despawn()
        {
            ...
            if (_prefabKey != null)
                BulletPool.Return(_prefabKey, gameObject);
            else
                Destroy(gameObject);      // 레거시 폴백 (풀 거치지 않은 경로)
        }
    }
}
